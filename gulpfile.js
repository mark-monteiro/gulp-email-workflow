...
gulp.task('insert-css', ['minify-css'], function() {
    var campaignCssContent = fs.readFileSync("campaign.min.css", "utf8");

    return gulp.src('campaign.html')
        .pipe(replace('{{campaign-css}}', campaignCssContent))
...
gulp.task('inline-css', ['insert-css'], function() {
    return gulp.src(['./build/*.html', '!./build/*-inlined.html'])
        .pipe(inlineCss({
            applyStyleTags: true,
            applyLinkTags: true,
...
gulp.task('watch', function() {
    gulp.watch(['campaign.html', 'campaign.less'], ['build']);
});
